...
function ajustarBrillo(matriz, factor) {
  ...
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
      resultado[i][j].r = limitarValorColor(Math.round(resultado[i][j].r * factor));
      resultado[i][j].g = limitarValorColor(Math.round(resultado[i][j].g * factor));
      resultado[i][j].b = limitarValorColor(Math.round(resultado[i][j].b * factor));
...
validarMatriz(matriz);
  ...
  for (let i = 0; i < resultado.length; i++) {
    resultado[i] = resultado[i].slice().reverse(); // invierte cada fila
  }
...
function rotar90Grados(matriz) {
  ...
  // Crear nueva matriz rotada (ancho x alto)
  const rotada = [];
...
validarMatriz(matriz1);
  ...
  const dims1 = obtenerDimensionesImagen(matriz1);
  const dims2 = obtenerDimensionesImagen(matriz2);
  ...
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
      ...
      resultado[i][j] = {
        r: limitarValorColor(Math.floor(p1.r * (1 - factor) + p2.r * factor)),
        g: limitarValorColor(Math.floor(p1.g * (1 - factor) + p2.g * factor)),
        b: limitarValorColor(Math.floor(p1.b * (1 - factor) + p2.b * factor)),
        a: limitarValorColor(Math.floor(p1.a * (1 - factor) + p2.a * factor)),
      };
    }
  }
...
function detectarBordes(matriz, umbral = 50) {
  ...
  // 1. Primero escala de grises
  const grises = convertirEscalaGrises(matriz);

  const alto = grises.length;
  const ancho = grises[0].length;

  // 2. Crear matriz resultado en negro
  ...
  // 3. Comparación simple con vecino derecho e inferior
  for (let y = 0; y < alto - 1; y++) {
    for (let x = 0; x < ancho - 1; x++) {
      const p = grises[y][x].r;
      const derecha = grises[y][x + 1].r;
      const abajo = grises[y + 1][x].r;

      const diff = Math.max(Math.abs(p - derecha), Math.abs(p - abajo));

      if (diff > umbral) {
        resultado[y][x] = { r: 255, g: 255, b: 255, a: 255 }; // borde blanco
      }
    }
  }
